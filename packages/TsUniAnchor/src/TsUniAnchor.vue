...
import { AnchorType, AnchorPosition } from '@/commonJs/type/AnchorType'

@Component
export default class TsUniAnchor extends Vue {
  // 锚点列表
  @Prop({
    default: () => { return [] }
  })
  private anchorList!: Array<AnchorType>;
  ...
  private dynamicAnchor?: boolean;
  // 是否分步滑动页面
  @Prop({
    default: false
  })
  private stepShow?: boolean;
  ...
  private scrollAlwaysComputePosition?: boolean;
  // 是否展示锚点全称
  private showFullName = true
  // 存放各锚点位置
  private anchorPosition: Array<AnchorPosition> = []
  // 当前定位的锚点
  ...
  // 点击锚点进行页面定位
  private toAnchor(anchor: AnchorType) {
    ...
    if (toElement) {
      ...
      const scrollTop = (toElement as any).offsetTop + this.offSet + headerHeight
  ...
  // 分步滑动页面进行定位
  private byStepShow(scrollTop: number) {
  ...
  private onScroll() {
      ...
      this.anchorList.forEach(anchor => {
        ...
        if (toElement) {
          this.anchorPosition.push({
            id: anchor.id,
            position: (toElement as any).offsetTop + (this.fixedHeader ? -this.headerHeight : this.headerHeight)
          })
        }
      })
    }
    ...
    // 将锚点位置列表根据锚点位置排序
    this.anchorPosition.sort(function(pre: AnchorPosition, next: AnchorPosition) { return pre.position - next.position })
    // 根据页面位置进行当前锚点高亮
